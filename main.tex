...
events, ?? to ??, are selected. Wire chamber tracks must pass the ``picky
track'' cuts, which means that there is exactly one hit on each X and Y-plane
in each of the four WC stations. This is required to ensure the purest sample
of tracks with the best momentum resolution. Trac

In the simulation, primary particles are generated from
the last WC. The assumed WC momentum and direction is set equal to the initial
momentum and direction of the generated primary particle. Momentum cuts and
WC-TPC track matching is then performed in MC just as in data, using the true
initial particle information on position, direction, and momentum as the WC
values.
...
%#
%#For pions:
%#WC p (or MC start p) in 100 to 1100 MeV
%#First TOF > 0 and < 25
%#
%#For protons
%#WC p (or MC start p) in 450 to 1100 MeV
%#First TOF > 28 and < 55
%#
%#N tracks in first 2cm in z: >=1
%#N Tracks in first 14cm in z: < 4
%#N tracks with length less than 5 cm: < 3
%#
%#Matching procedure...
%#Only one matched track

\begin{table}[!hbtp]
  \begin{center}
    \caption{Primary \pip{} selection cuts.}
    \label{tab:pipCuts}
    \begin{tabu}{|l|l|} \hline
      Variable & Cut \\ \hline \hline
      WC momentum & \SIrange[range-phrase={ to }]{100}{1100}{\MeVc{}} \\ \hline
      First ToF & \SIrange[range-phrase={ to }]{0}{25}{\nano \second} \\ \hline
